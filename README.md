...
To install the app, you need to:
	...
	cmake --install build -DCMAKE_INSTALL_PREFIX="<your_path/friction_problem/install>"
	
	# Next, it remains to build the application itself, indicating where to look for
	# the matplotlib-cpp config files..
	cd ../../
	cmake -S . -B build -DCMAKE_PREFIX_PATH="<your_path>friction_problem/install/matplotlibcpp/lib/matplotlib_cpp/cmake"
	cmake --build build --config Release
	```
